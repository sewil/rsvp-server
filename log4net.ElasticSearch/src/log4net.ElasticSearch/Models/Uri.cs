...
namespace log4net.ElasticSearch.Models
{
    public class Uri
    {
        ...
        public static implicit operator System.Uri(Uri uri)
        {
            ...
            {
                return
                    new System.Uri($"{uri.Scheme()}://{uri.User()}:{uri.Password()}@{uri.Server()}:{uri.Port()}/{uri.Index()}{uri.Bulk()}");
            }
            return string.IsNullOrEmpty(uri.Port())
                ? new System.Uri($"{uri.Scheme()}://{uri.Server()}/{uri.Index()}{uri.Bulk()}")
                : new System.Uri($"{uri.Scheme()}://{uri.Server()}:{uri.Port()}/{uri.Index()}{uri.Bulk()}");
        }
