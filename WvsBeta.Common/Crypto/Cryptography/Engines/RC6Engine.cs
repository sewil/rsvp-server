...
namespace Org.BouncyCastle.Crypto.Engines
{
    ...
    */
    ...
    {
        private static readonly int _WordSize = 32;
        private static readonly int _BytesPerWord = _WordSize / 8;
        ...
        */
        ...
        public virtual int GetBlockSize() => 4 * _BytesPerWord;
        ...
        */
        private void SetKey(byte[] key)
        {
            ...
            int c = (key.Length + (_BytesPerWord - 1)) / _BytesPerWord;
            if (c == 0)
            {
                c = 1;
            }
            int[] L = new int[(key.Length + _BytesPerWord - 1) / _BytesPerWord];

            // load all key bytes into array of key dwords
            for (int i = key.Length - 1; i >= 0; i--)
            {
                L[i / _BytesPerWord] = (L[i / _BytesPerWord] << 8) + (key[i] & 0xff);
            }
        ...
        private int EncryptBlock(byte[] input, int inOff, byte[] outBytes, int outOff)
        {
            ...
            int B = BytesToWord(input, inOff + _BytesPerWord);
            int C = BytesToWord(input, inOff + _BytesPerWord * 2);
            int D = BytesToWord(input, inOff + _BytesPerWord * 3);
            ...
            WordToBytes(B, outBytes, outOff + _BytesPerWord);
            WordToBytes(C, outBytes, outOff + _BytesPerWord * 2);
            WordToBytes(D, outBytes, outOff + _BytesPerWord * 3);

            return 4 * _BytesPerWord;
        }
        ...
        */
        private int RotateLeft(int x, int y)
            => ((int)((uint)(x << (y & (_WordSize - 1))) | ((uint)x >> (_WordSize - (y & (_WordSize - 1))))));
        ...
        */
        ...
        private int BytesToWord(byte[] src, int srcOff)
        {
            ...
            for (int i = _BytesPerWord - 1; i >= 0; i--)
            {
