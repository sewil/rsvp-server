...
namespace WvsBeta.Common
{
    public static class Constants
    {
        ...
        public const byte MaxBuddyListCapacity = 150;
        ...
        public const int InvisibleMob = 9999999;
        public const int InvalidMap = 999999999;
        public const int MapleAdminNpc = 9010000;

        
        public const int Zakum3 = 8800002;
        ...
        {
            public const int PetFood = 2120000;
            ...
            public const int SnowWeather = 2090000;

            public const int ItemMegaphone = 2081000;
            public const int ItemSuperMegaphone = 2082000;

            public const int ItemAPReset = 2180000;

            public static class Types
            {
                // WeaponTypes is basically ItemType % 100
        ...
        public static Items.Types.ItemTypes getItemType(int itemid) => (Items.Types.ItemTypes)(itemid / 10000);
        public static Items.Types.WeaponTypes getWeaponType(int itemid) => (Items.Types.WeaponTypes)((itemid / 10000) % 100);
        public static int getScrollType(int itemid) => (itemid % 10000) - (itemid % 100);
        public static int itemTypeToScrollType(int itemid) => (int) getItemType(itemid) % 100 * 100;
        public static bool isArrow(int itemid) => getItemType(itemid) == Items.Types.ItemTypes.ItemArrow;
        public static bool isStar(int itemid) => getItemType(itemid) == Items.Types.ItemTypes.ItemStar;
        public static bool isRechargeable(int itemid) => isStar(itemid);
        public static bool isEquip(int itemid) => getInventory(itemid) == 1;
        public static bool isMonsterBook(int itemid) => getItemType(itemid) == Items.Types.ItemTypes.ItemMonsterBook;
        public static bool isInternetCafeMouse(int itemid) => itemid == 4000047;
        public static bool isPet(int itemid) => getInventory(itemid) == 5;
        public static bool isStackable(int itemid) => !(isRechargeable(itemid) || isEquip(itemid) || isPet(itemid));
        public static bool isOverall(int itemid) => getItemType(itemid) == Items.Types.ItemTypes.ArmorOverall;
        public static bool isTop(int itemid) => getItemType(itemid) == Items.Types.ItemTypes.ArmorTop;
        public static bool isBottom(int itemid) => getItemType(itemid) == Items.Types.ItemTypes.ArmorBottom;
        public static bool isShield(int itemid) => getItemType(itemid) == Items.Types.ItemTypes.ArmorShield;
        public static bool is2hWeapon(int itemid) => (int) getItemType(itemid) / 10 == 14;
        public static bool is1hWeapon(int itemid) => (int) getItemType(itemid) / 10 == 13;
        public static bool isWeaponCover(int itemid) => getItemType(itemid) == Items.Types.ItemTypes.WeaponCash;
        public static bool isBow(int itemid) => getItemType(itemid) == Items.Types.ItemTypes.WeaponBow;
        public static bool isCrossbow(int itemid) => getItemType(itemid) == Items.Types.ItemTypes.WeaponCrossbow;
        public static bool isSword(int itemid) => getItemType(itemid) == Items.Types.ItemTypes.Weapon1hSword || getItemType(itemid) == Items.Types.ItemTypes.Weapon2hSword;

        public static bool isMace(int itemid)
        {
            return getItemType(itemid) == Items.Types.ItemTypes.Weapon1hMace || getItemType(itemid) == Items.Types.ItemTypes.Weapon2hMace;
        }

        public static bool isValidInventory(byte inv) => inv > 0 && inv <= 5;

        public static bool isPuppet(int skillid) => skillid == Sniper.Skills.Puppet || skillid == Ranger.Skills.Puppet;
        public static bool isSummon(int skillid) => isPuppet(skillid) || skillid == Priest.Skills.SummonDragon || skillid == Ranger.Skills.SilverHawk || skillid == Sniper.Skills.GoldenEagle;

        // This should actually be loaded of skill level data (mastery field)
        public static byte getMasteryDisplay(byte level) => (byte) ((level + 1) / 2);

        public static short getJobTrack(short job, bool flatten = false) => (short) (flatten ? (job / 100 % 10) : (job / 100));
        ...
        /// <returns></returns>
        public static int get_job_level(int job)
        {
            if (job < 100) return 0;
        ...
        /// <summary>
        /// Returns a list of jobs that have been progressed through.
        /// For 421, it returns 400, 420, 421.
        /// </summary>
        /// <param name="job"></param>
        /// <returns></returns>
        public static IEnumerable<int> get_skill_root_from_job(int job)
        {
            var jobLevel = get_job_level(job);
            if (jobLevel == 0) yield break;

            var firstJob = job / 100 * 100;
            if (jobLevel >= 1) yield return firstJob;

            // Take the second digit from the job
            var secondJob = firstJob + (job / 10 % 10 * 10);
            if (jobLevel >= 2) yield return secondJob;

            if (jobLevel >= 3) yield return secondJob + 1;
        }
        ...
        public static bool isRequiredJob(short jobTrack, short reqJob)
        {
            // Special code for all-jobs items
            if (reqJob == 0) return true;

            JobFlags job = jobTrack switch
            {
                0 => JobFlags.Beginner,
                1 => JobFlags.Warrior,
                2 => JobFlags.Mage,
            ...
            if (reqJob == -1)
            {
                // Special code for beginner-only items
                return job == JobFlags.Beginner;
            }

            return ((short)job & reqJob) == (short)job;
        }
        ...
        public static class Sniper
        {
            ...
            public static class Skills
            {
                public const int Iceshot = 3211003;
