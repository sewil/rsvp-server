...
namespace WvsBeta.SharedDataProvider
{
    ...
    public class CharacterCashItems
    {
        ...
        public LockerItem GetLockerItemFromCashID(long cashId) => Items.FirstOrDefault(x => x.CashId == cashId && x.UserId == UserID);
        ...
        public void RemoveLockerItem(LockerItem li, BaseItem item, bool deleteFromDB)
        {
            ...
            if (deleteFromDB && li.SavedToDatabase) DeletedCashItems.Add(li.CashId);
        }

        private long lastCheck = 0;

        public void GetExpiredItems(long fileTime, Action<List<LockerItem>> callback)
        {
            ...
            callback(Items.Where(x => x.Expiration < fileTime).ToList());
        }
    }
}