...
CREATE TABLE IF NOT EXISTS `{PATCHES_TABLE}` (
   ...
   `script_down` longtext NULL DEFAULT NULL,
...
ALTER TABLE `{PATCHES_TABLE}` MODIFY `script_down` longtext NULL DEFAULT NULL;
";
            ...
            */
            ...
            for (var i = maxPatches - 1; i >= 0; i--)
            {
                ...
                else
                {
                    ...
                    if (local.FileHash != db.FileHash || db.State == PatchFile.PatchingState.AppliedDown)
                    {
                        for (var j = maxPatches - 1; j >= i; j--)
                        {
                            local = patchesOnDisk[j];
                            db = patchesInDB[j];
