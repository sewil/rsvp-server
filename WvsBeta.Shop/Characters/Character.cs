...
namespace WvsBeta.Shop
{
    public class Character
    {
        ...
        public const string CouponUsedTransactionText = "CouponUsed: ";
        public const string CouponUsedTransactionType = "nx";
        ...
        public byte TrunkSlotCount { get; set; }
        ...
        public List<string> UsedCoupons { get; } = new List<string>();
        ...
        public void Save()
        {
            Server.Instance.CharacterDatabase.RunTransaction(comm =>
            {
                ...
                comm.CommandText = "UPDATE storage SET slots = " + TrunkSlotCount + " WHERE userid = " + UserID;
                comm.ExecuteNonQuery();
        ...
        public LoadFailReasons Load(string IP)
        {
            ...
            {
                int i = 0;
                while (data.Read() && i < 10)
                {
                    Wishlist[i++] = data.GetInt32(0);
                }
            }

            // Note: when you don't load skills, you lose your keymap.
            ...
            Server.Instance.CharacterDatabase.RunQuery("INSERT IGNORE INTO `storage` (userid, world_id) VALUES (@userid, @worldid)", "@userid", UserID, "@worldid", Server.Instance.WorldID);

            using (var data = (MySqlDataReader)Server.Instance.CharacterDatabase.RunQuery("SELECT slots FROM storage WHERE userid = @userid AND world_id = @worldid", "@userid", UserID, "@worldid", Server.Instance.WorldID))
            {
                if (!data.Read())
                    throw new Exception("Unable to read storage record???");

                TrunkSlotCount = (byte)data.GetInt32(0);
            }

            using (var data = (MySqlDataReader)Server.Instance.CharacterDatabase.RunQuery(
                $"SELECT note FROM user_point_transactions WHERE userid = @userid AND note LIKE '{CouponUsedTransactionText}%' AND pointtype = '{CouponUsedTransactionType}'",
                "@userid", UserID
            ))
            {
                while (data.Read())
                {
                    var note = data.GetString(0);
                    UsedCoupons.Add(note.Substring(CouponUsedTransactionText.Length));
                }
            }
