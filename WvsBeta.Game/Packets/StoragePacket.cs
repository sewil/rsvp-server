...
namespace WvsBeta.Game
{
    public static class StoragePacket
    {
        ...
        public static void HandleStorage(Character chr, Packet pr)
        {
            ...
            switch ((StorageAction)opcode)
            {
                case StorageAction.Withdraw: // Remove
                    {
                        var inventory = pr.ReadByte();
                        var slot = pr.ReadByte();
                        var item = chr.Storage.GetItem(inventory, slot);
                        if (item == null)
                        {
                            return;
                        }
                        ...
                        if (chr.Inventory.HasSlotsFreeForItem(item.ItemID, amount))
                        {
                            ...
                            chr.Inventory.DistributeItemInInventory(item);
                ...
                case StorageAction.Deposit: // Add
                    {
                        var slot = (byte)pr.ReadShort();
                        var itemid = pr.ReadInt();
                        var amount = pr.ReadShort();
                        var data = DataProvider.NPCs[chr.TrunkNPCID];
                        ...
                        var item = chr.Inventory.GetItem(inventory, slot);
                        ...
                        var possibleNewItem = chr.Inventory.TakeItemAmountFromSlot(inventory, slot, amount);
        ...
        {
            var pw = new Packet(ServerMessages.STORAGE);
            pw.WriteInt(NPCID);
        ...
        {
            var pw = new Packet(ServerMessages.STORAGE_RESULT);
            pw.WriteByte(14);
        ...
        {
            var packet = new Packet(ServerMessages.STORAGE_RESULT);
        ...
        private static void EncodeStorage(Character chr, Packet packet, StorageEncodeFlags flags)
        {
            ...
            for (byte i = 1; i <= 5; i++)
            {
                var flag = GetEncodeFlagForInventory(i);
