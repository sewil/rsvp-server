...
namespace WvsBeta
{
    public partial class MasterThread
    {
        public class RepeatingAction
        {
            ...
            public static RepeatingAction Start(string pName, Action<long> pAction, TimeSpan start, TimeSpan repeat, Func<bool> pEndCondition = null) =>
                Start(pName, pAction, (long)start.TotalMilliseconds, (long)repeat.TotalMilliseconds, pEndCondition);

            public static RepeatingAction Start(string pName, Action pAction, TimeSpan start, TimeSpan repeat, Func<bool> pEndCondition = null) =>
                Start(pName, _ => pAction(), start, repeat, pEndCondition);
            ...
            /// <param name="repeat">Amount of milliseconds between each run. Use 0 to have a one-time action.</param>
            /// <param name="pEndCondition">When this returns true, the repeating action will be deregistered.</param>
            public static RepeatingAction Start(string pName, Action pAction, long start, long repeat, Func<bool> pEndCondition = null) =>
                Start(pName, x => pAction(), start, repeat, pEndCondition);
            ...
            private void StartTask()
            {
                ...
                _task.Start();
#else
                _task = Task.Factory.StartNew(RunTask, TaskCreationOptions.LongRunning);
#endif
            }
            ...
            {
                return string.Format("RepeatingAction. Name: {2}, Time for each run: {0}, Next run: {1}", TimeSpan.FromMilliseconds(Repeat), TimeSpan.FromMilliseconds(NextRun - CurrentTime), Name);
            }
        }
